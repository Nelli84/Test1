		...
		uiStage.addListener(new InputListener(){
			...
			public boolean touchDown(InputEvent event, float x, float y,
				...
				if ( y < uiStage.getHeight() * .5f) {
					...
					if (x <= uiStage.getWidth() * .1f) {
						runnable1 = runnableGoLeft;
						
					} else if (x <= uiStage.getWidth() * .2f) {
						runnable1 = runnableGoRight;
					}
...
//				265 * WORLD_TO_SCREEN * scale, 137 * WORLD_TO_SCREEN * scale);
...
//		Gdx.gl.glClearColor(BACKGROUND_COLOR.r, BACKGROUND_COLOR.g, BACKGROUND_COLOR.b, BACKGROUND_COLOR.a);
		...
		if (Gdx.input.isTouched()) {
			...
			uiStage.getCamera().unproject(tmp);

			if (tmp.y < uiStage.getHeight() * .5f) {
				Gdx.app.debug("index", "blow ");
				if (tmp.x < uiStage.getWidth() * .2f) {
					goLeft();
				} else if (tmp.x < uiStage.getWidth() * .4f) {
					goRight();
				}
			}
		}
...
//		gr1Body.createFixture(gr1FDef);
	...
	private void createBall(){
	    ...
	    // 4. Create the body fixture automatically by using the loader.
	    loader.attachFixture(body, "circle", fDef, 2 * radius);
...
//	    loader.attachFixture(body, "ball", fDef, 2 * radius);
	...
	private void createSoftBall_(){
	    ...
	    float radius_ =  6.5f * WORLD_TO_SCREEN;
	    ...
	    fDef.density = 0;
	    fDef.isSensor = true;
	    fDef.filter.categoryBits = BIT_FOR_WATER;
...
//	    fixtureDef.restitution = 0.05f;
	    ...
	    for (int i = 0; i < segmentsCount; i++) {
	     ...
	     // 4. Create the body fixture automatically by using the loader.
		    loader.attachFixture(body, "circle", fDef, 2 * radius_);
...
//	        [bodies addObject:[NSValue valueWithPointer:body]];
	    ...
	    // 4. Create the body fixture automatically by using the loader.
	    loader.attachFixture(innerCircleBody, "circle", fDef, 2 * radius_);
...
//				b.applyLinearImpulse(direction.scl(5),b.getWorldCenter(), true);
				b.applyForceToCenter(direction.scl(.72f), true);
	...
	private void createWater(float x, float y, float width, float height){
		...
		world.setContactListener(new ContactListener() {
			...
			public void preSolve(Contact contact, Manifold oldManifold) {
				...
				if("water".equals(contact.getFixtureA().getUserData()) || "water".equals(contact.getFixtureB().getUserData()) ){
			...
			public void endContact(Contact contact) {
				...
				if("water".equals(contact.getFixtureA().getUserData())){// && "forSensor".equals(contact.getFixtureB().getUserData())){
                	removeFromPairs(contact.getFixtureA(), contact.getFixtureB());
                	contact.getFixtureB().getBody().setLinearDamping(contact.getFixtureB().getBody().getLinearDamping() - damping);
					contact.getFixtureB().getBody().setAngularDamping(contact.getFixtureB().getBody().getAngularDamping() - damping);
				}
				if("water".equals(contact.getFixtureB().getUserData())){// && "forSensor".equals(contact.getFixtureA().getUserData()) ){
					removeFromPairs(contact.getFixtureB(), contact.getFixtureA());
					contact.getFixtureA().getBody().setLinearDamping(contact.getFixtureA().getBody().getLinearDamping() - damping);
					contact.getFixtureA().getBody().setAngularDamping(contact.getFixtureA().getBody().getAngularDamping() - damping);
				}
			...
			public void beginContact(Contact contact) {
				...
				if("water".equals(contact.getFixtureA().getUserData())){// && "forSensor".equals(contact.getFixtureB().getUserData()) ){
                	pairs.add(new Pair(contact.getFixtureA(), contact.getFixtureB()));
                	contact.getFixtureB().getBody().setLinearDamping(contact.getFixtureB().getBody().getLinearDamping() + damping);
					contact.getFixtureB().getBody().setAngularDamping(contact.getFixtureB().getBody().getAngularDamping() +damping);
				}
				if("water".equals(contact.getFixtureB().getUserData())){// && "forSensor".equals(contact.getFixtureA().getUserData())){
					pairs.add(new Pair(contact.getFixtureB(), contact.getFixtureA()));
					contact.getFixtureA().getBody().setLinearDamping(contact.getFixtureA().getBody().getLinearDamping() + damping);
					contact.getFixtureA().getBody().setAngularDamping(contact.getFixtureA().getBody().getAngularDamping() +damping);
				}
	...
	private void detectWaterCollision(){
		for(Pair pair : pairs){
		      ...
		      if ( (result = findIntersectionOfFixtures(pair.first, pair.second)) != null ) {
        		...
        		Gdx.app.debug("index", "applyForceToCenter centroid.area = " + area);
		      }
		}
	}
	...
	private static class Pair{
	 ...
	 //////////////////////////////////////////////////////
	  ...
	  private static List<float[]> findIntersectionOfFixtures(Fixture fA, Fixture fB){
		  // these subject and clip points are assumed to be valid
//		  float[][] subjPoints = {{50, 150}, {200, 50}, {350, 150}, {350, 300},
//				  {250, 300}, {200, 250}, {150, 350}, {100, 250}, {100, 200}};
//		  
//		  float[][] clipPoints = {{100, 100}, {300, 100}, {300, 300}, {100, 300}};
	  ...
	  static private float[] intersection(float[] a, float[] b, float[] p, float[] q) {
	        float A1 = b[1] - a[1];
	        float B1 = a[0] - b[0];
